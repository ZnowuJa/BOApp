...
{
    public class CreateBusinessTripCommand(BusinessTravelFormVm item) : IRequest<int>
    {
        public BusinessTravelFormVm Item { get; set; } = item;

    }

    public class CreateBusinessTripCommandHandler(IAppDbContext context, IMapper mapper, IEmailService mailService, IConfiguration configuration) : IRequestHandler<CreateBusinessTripCommand, int>
    {
        ...
        public async Task<int> Handle(CreateBusinessTripCommand request, CancellationToken cancellationToken)
        {
            var item = _mapper.Map<BusinessTravelForm>(request.Item);
            _appDbContext.BusinessTravels.Add(item);
