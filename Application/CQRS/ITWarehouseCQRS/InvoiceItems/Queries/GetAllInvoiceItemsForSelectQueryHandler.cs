...
internal class GetAllInvoiceItemsForSelectQueryHandler : IRequestHandler<GetAllInvoiceItemsForSelectQuery, IQueryable<InvoiceItemVm>>
{
    private readonly IAppDbContext _appDbContext;
    private IMapper _mapper;

    public GetAllInvoiceItemsForSelectQueryHandler(IAppDbContext appDbContext, IMapper mapper)
    {
    ...
    public async Task<IQueryable<InvoiceItemVm>> Handle(GetAllInvoiceItemsForSelectQuery request, CancellationToken cancellationToken)
    {
        InvoiceItemVm inv = new InvoiceItemVm() { Id = 0, Name = "Select..."};
        List<InvoiceItemVm> listItem = [inv];
